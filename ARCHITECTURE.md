...
- **`services/simulation/recommend.ts`**: no inventa equipos — filtra `CATALOG_DATA` (el mismo catálogo de `/app/catalog`) y elige panel/inversor/batería/**transformador** reales que mejor ajustan al requerimiento calculado, devolviendo el arreglo de razones (`reasons: string[]`) que la UI muestra tal cual:
  ...
  - **Medición** (`recommendMetering`): a partir de la capacidad instalada aproximada (potencia AC del inversor ÷ factor de potencia asumido 0.9) y del nivel de tensión (BT si no hay transformador, MT/AT si lo hay), recomienda el tipo de medición (directa / semidirecta / indirecta) y la clase de exactitud, citando la fuente real: **Tabla 5 de la norma técnica RA8-030** ("Selección y conexión de equipos del sistema de medida de energía eléctrica", norma conjunta de Grupo EPM/ESSA/EDEQ/CENS/CHEC), que a su vez se basa en la **NTC 5019:2018** y en el **Código de Medida (Resolución CREG 038 de 2014)**. Se investigó el documento real (no se inventó el umbral): medición directa en BT hasta ~15 kVA (aproximación de "X", que la norma define como la corriente máxima del medidor de conexión directa disponible, ≥60 A según Tabla 6 de RA8-030), semidirecta en BT entre ese umbral y 100 kVA, e indirecta en MT/AT o cuando la conexión se hace a través de un transformador de potencia (el punto de medida se ubica en el lado de alta tensión, RA8-030 §6.8.a). Sigue siendo una referencia: cada operador de red aplica su propia norma técnica equivalente.
...
- **`services/calculations/conductor.ts`**: dimensionamiento de conductores DC y AC. Tabla de referencia de calibres AWG (sección, resistencia y ampacidad a ~75°C, ≤3 conductores, equivalente a NTC 2050/NEC 310.16). `recommendConductor()` elige el calibre más delgado que cumple simultáneamente (a) ampacidad ≥ corriente × 1.25 de margen (criterio NEC 690.8/RETIE para circuitos FV de operación continua) y (b) caída de tensión objetivo, usando la fórmula de Guía Práctica §8 (factor 2 por ida/vuelta en DC y AC monofásica, √3 en AC trifásica). Si ningún calibre de la tabla cumple la caída de tensión, devuelve el más grueso y lo marca explícitamente en vez de fingir que cumple.
...
- **`components/engineering/ArraySceneViewer.tsx`**: vista 3D interactiva con `@react-three/fiber` + `@react-three/drei`. Dibuja el mismo resultado de `layout.ts` pero en volumen: los paneles se inclinan según el ángulo indicado y el grupo completo rota según el azimut (orientación). Cada equipo (arreglo, inversor, batería, transformador, **medidor**) se pasa como un objeto `EquipmentInfo { label, detail, color }` — `label` es el modelo real del catálogo (o "Medidor" con el tipo de medición) y `detail` es su dato clave, mostrado en una etiqueta flotante (`Html` de drei) sobre cada marcador. Cámara orbital (`OrbitControls`) para rotar y hacer zoom.
- **`components/engineering/SingleLineDiagram.tsx`**: diagrama unifilar en SVG — cadena Arreglo FV → Protecciones DC → Inversor → Protecciones AC → (Transformador) → Red/Medidor, con la Batería como rama conectada al inversor cuando aplica. El nodo del arreglo muestra la configuración de strings (ej. "2× 12 en serie"), el nodo final muestra el tipo de medidor recomendado en vez de un texto genérico. Cada nodo muestra categoría, modelo real y especificación clave (truncados si exceden el ancho de la caja). Es un diagrama de topología, no calcula corrientes de cortocircuito.
- **`services/simulation/installationFlow.ts`** + **`components/engineering/InstallationFlowDiagram.tsx`**: `buildInstallationFlow()` arma la secuencia de pasos de instalación (preparación → estructura → paneles → cableado DC → protecciones DC → inversor → batería si aplica → protecciones/cableado AC → transformador si aplica → **sistema de medición** (con la descripción exacta según directa/semidirecta/indirecta) → conexión a red → puesta a tierra → comisionamiento), variando el contenido según si hay batería/transformador, el tipo de medición y si la superficie es techo o patio. `InstallationFlowDiagram` lo renderiza como una línea de tiempo vertical numerada.

**Nota de honestidad técnica**: la vista 3D es una representación esquemática de la disposición y orientación del arreglo, no una simulación solar (no calcula sombras proyectadas por el sol real) ni una vista "4D" en el sentido de BIM (3D + cronograma de obra) — ese alcance sigue pendiente y se documenta en `TODO.md` si se quiere abordar más adelante. El diagrama unifilar tampoco reemplaza un diseño eléctrico certificado: no calcula corrientes de cortocircuito, solo representa la topología con los equipos y calibres recomendados.

## 5.1 Fuentes normativas consultadas para la medición

A diferencia del resto del proyecto (que solo cita el Manual Maestro y la Guía Práctica propios), la clasificación de medición se investigó contra documentación real del sector eléctrico colombiano, porque el usuario pidió explícitamente no usar una regla inventada:

- RA8-030 "Selección y conexión de equipos del sistema de medida de energía eléctrica", Rev. 4 (2021-04-09), norma técnica conjunta de Grupo EPM, ESSA, EDEQ, CENS y CHEC — Tabla 1 (clasificación de puntos de medición), Tabla 5 (selección de medidores por nivel de tensión y capacidad instalada) y Tabla 6 (características eléctricas de los medidores).
- Resolución CREG 038 de 2014 (Código de Medida) — definiciones de medición directa/semidirecta/indirecta y clasificación de puntos de medición por capacidad instalada.
- NTC 5019:2018 — selección de equipos de medición de energía eléctrica.
- Resolución CREG 030 de 2018 y CREG 174 de 2021 — autogeneración a pequeña escala y generación distribuida (medidor bidireccional con perfil horario).

Estas mismas fuentes están citadas dentro del texto de `recommendMetering()` para que quien use la app pueda verificarlas.
